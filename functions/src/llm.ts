...
const ASK_PLAN_SYS = [
  ...
  "Use dateFrom/dateTo for a date range on the table's own date field (listed above as \"date field\") --",
  "never also add a filters entry for that same field, it would just duplicate dateFrom/dateTo.",
  ...
  "",
  "One plan produces ONE shape: either a grouped count/sum (groupBy set, no columns) or a row list with",
  "specific columns (groupBy null). It cannot do both, so when a question asks for a count/breakdown AND",
  "row-level detail -- names, who submitted it, individual statuses -- in the same request, prefer the",
  "row-level table with groupBy null over the aggregate table: a row list still shows the breakdown when",
  "read down the column (e.g. sorted or eyeballed by client), but a grouped count can never show the names",
  'the question also asked for. Example: "requirements received per client along with the recruiter and',
  'candidate names and statuses" -- do not use the `clients` table (it has no candidate/recruiter/status',
  "columns and would silently drop half the question); use `submissions`, groupBy null, columns including",
  "client, recruiter, applicantName, status. Only use an aggregate table (recruiters/clients) when the",
  "question asks purely for counts/rates with no per-record detail.",
  "",
  ...
  "names in the data may include middle initials, suffixes, a longer legal name, or different spacing than",
  'the question uses, and "contains" still matches those; "eq" does not. This applies even when the value',
  'looks like a short, plain word -- e.g. a client called "cognizant" in the question must still be',
  '{ "field": "client", "op": "contains", "value": "cognizant" }, not "eq", because the stored name could be',
  '"Cognizant Technology Solutions". Only use "eq" for a value from a fixed set the catalog above enumerates',
  "(e.g. a status or enum column) where the question's value can only be that literal option.",
  "Output the JSON object and nothing else.",
].join("\n");
